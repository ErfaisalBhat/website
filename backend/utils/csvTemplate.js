...
/** Normalises an academicYear value to the "YYYY-YY" short form (e.g. "2026-27"). */
function normaliseAcademicYear(raw) {
  const s = String(raw || '').trim();
  // Already short: "2026-27"
  if (/^\d{4}-\d{2}$/.test(s)) return s;
  // Long form: "2026-2027"
  const m = s.match(/^(\d{4})-(\d{4})$/);
  if (m) return `${m[1]}-${m[2].slice(-2)}`;
  // Bare year: "2026" → "2026-27"
  const y = s.match(/^(\d{4})$/);
  if (y) {
    const yr = parseInt(y[1], 10);
    return `${yr}-${String(yr + 1).slice(-2)}`;
  }
  return s || String(new Date().getFullYear());
}
...
async function parseCertificateCSV(filePathOrBuffer) {
  ...
  records.forEach((row, idx) => {
    ...
    // NOTE: certificateNumber is intentionally omitted here —
    // it is generated by the controller using a DB-backed sequence.
    valid.push({
      ...
      academicYear: normaliseAcademicYear(row.AcademicYear),
